...
class FinancialCrew():
    ...
    def spending_advisor(self) -> Agent:
        return Agent(config=self.agents_config['spending_advisor'], verbose=True, allow_delegation=True)
    
    @agent
    def goal_tracker(self) -> Agent:
        return Agent(config=self.agents_config['goal_tracker'], verbose=True, allow_delegation=True)
    ...
    def agent_coordinator(self) -> Agent:
        return Agent(config=self.agents_config['agent_coordinator'], verbose=True, allow_delegation=True)
    ...
    def mentor_agent(self) -> Agent:
        return Agent(config=self.agents_config['mentor_agent'], verbose=True, allow_delegation=True)
    
    @agent
    def monthly_summary_agent(self) -> Agent:
        return Agent(config=self.agents_config['monthly_summary_agent'], verbose=True, allow_delegation=True)
    ...
    def financial_strategy_task(self) -> Task:
        return Task(config=self.tasks_config['financial_strategy_task'],tools=[debt_repayment_tool, portfolio_simulation_tool, emergency_fund_tool])
    
    @task
    def coordinator_decision(self) -> Task:
        return Task(config=self.tasks_config['coordinator_decision'])
    ...
    def mentor_task(self) -> Task:
        return Task(config=self.tasks_config['mentor_task'])
    ...
    def monthly_summary(self) -> Task:
        return Task(config=self.tasks_config['monthly_summary'])
    ...
    def flexible_crew(self, input_data=None, agent_overrides=None, task_overrides=None) -> Crew:
        ...
        financial_strategy_task.context = [simulate_task]
        coordinator_task.context = [evaluate_task, goal_task, financial_strategy_task]
        discipline_tracker_task.context = [evaluate_task, goal_task, financial_strategy_task, coordinator_task]
        mentor_task.context = [evaluate_task, goal_task, financial_strategy_task, coordinator_task, discipline_tracker_task]
        karma_tracker_task.context = [evaluate_task, goal_task, financial_strategy_task , coordinator_task, discipline_tracker_task, mentor_task]
        behaviour_tracker_task.context = [evaluate_task, goal_task, financial_strategy_task , coordinator_task, discipline_tracker_task, mentor_task]
        summary_task.context = [evaluate_task, goal_task, financial_strategy_task , coordinator_task, discipline_tracker_task, mentor_task, karma_tracker_task, behaviour_tracker_task]
